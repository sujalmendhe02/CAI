...
def register(entries, required, menu_var):
    ...
    if(len(no_face) > 0):
        ...
        if(rowId >= 0):
            ...
            # Save profile pic with criminal name for easier retrieval
            cv2.imwrite(f"profile_pics/{entry_data['Name'].lower()}.png", img_list[profile_img_num])
            # Also save with the old format for backward compatibility
            cv2.imwrite("profile_pics/criminal_%d.png"%rowId, img_list[profile_img_num])
...
def showCriminalProfile(name):
    print(f"Looking for criminal profile: '{name}'")  # Debug print
    ...
    # Retrieve actual criminal data
    crim_data = retrieveData(name)
    
    
    if crim_data is None:
        print(f"No data found for: '{name}', using default data")  # Debug print
        crim_data = {
            "Name": name,
            "Father's Name": "Not Available",
            "Gender": "Not Available",
            "DOB(yyyy-mm-dd)": "Not Available",
            "Crimes Done": "Not Available"
        }
    print(f"Retrieved data: {crim_data}")  # Debug print
    # Try to load the actual profile picture
    profile_pic_path = None
    
    # Check for profile picture in different possible locations
    possible_paths = [
        f"profile_pics/{name.lower()}.png",
        f"profile_pics/{name.lower().strip()}.png",
        f"profile_pics/criminal_{name.lower()}.png",
        f"profile_pics/{name}.png",
        f"profile_pics/criminal_1.png"  # Fallback to numbered format
    ]
    
    # Also check if there are any numbered profile pics
    if os.path.exists("profile_pics"):
        for file in os.listdir("profile_pics"):
            if name.lower() in file.lower():
                possible_paths.append(f"profile_pics/{file}")
                print(f"Found potential profile pic: {file}")  # Debug print
    
    profile_img = None
    for path in possible_paths:
        if os.path.exists(path):
            profile_img = cv2.imread(path)
            print(f"Loaded profile image from: {path}")  # Debug print
            break
    
    if profile_img is None:
        print("No profile image found, using placeholder")  # Debug print
    
    # If no profile picture found, create a placeholder
    if profile_img is None:
        profile_img = np.zeros((400, 400, 3), dtype=np.uint8)
        profile_img.fill(50)  # Dark gray background
        cv2.putText(profile_img, "No Image", (120, 180), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
        cv2.putText(profile_img, "Available", (130, 220), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
    else:
        # Resize the image to a standard size
        profile_img = cv2.resize(profile_img, (400, 400))
    ...
    for i, item in enumerate(crim_data.items()):
        ...
        val = "Not Available" if (item[1]=="" or item[1] is None) else item[1]
